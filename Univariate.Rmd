...
# a = arima(train_subset99, c(2,1,3), c(2,1,2))
# a_stat = c(a$sigma2, a$loglik, a$aic)
...
table1 = rbind(b_stat, c_stat, d_stat, e_stat, f_stat, h_stat)
colnames(table1) = c('Sigma2', 'loglikelihood', 'AIC')
table1
```

It seems that e and f have the more optimal values of the models that have been used for training.
We test if h is better than e using the likelihood ratio test.
###Likelihood Ratio Test:
```{r}
...
From this we can say that SARIMA(1,1,3)(2,1,3)[12] does better than the other models. We first do a residual diagnostic to check if our model is a good fit on the data.


```{r}
# test whether residuals have zero mean
(t.test(f$residuals))
plot(f$residuals, main = "Residuals vs. Time", ylab = "Residuals")
abline(h = 0, col = "red")
```
...
Test for homoscedasticity
```{r}
group <- c(rep(1,30),rep(2,30),rep(3,30), rep(4,30), rep(5,30), rep(6,30))


(leveneTest(f$residuals,group)) #Levene
#(bartlett.test(g$residuals,group)) #Bartlett  
scatter.smooth(e$residuals, ylab = 'Residuals')
```
Test for normality:
...
forecast_sarima_subset = forecast(e, h = 24, level = 95)
plot(forecast_sarima_subset)
valid
forecast_sarima_subset
rmse_sarima_subset = rmse(valid, forecast_sarima_subset$mean)
rmse_sarima_subset
```
...
plot(forecast_te)
forecast_te
rmse_te = rmse(valid, forecast_te$mean)
rmse_te
```
