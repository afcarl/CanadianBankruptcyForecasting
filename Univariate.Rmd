...
```{r}
only_bankruptcy = ts(train$Bankruptcy_Rate, start = c(2010,7), frequency = 12)
only_bankruptcy
...
only_bankruptcy_differenced = diff(only_bankruptcy, differences = d)
print(d)
print(pvalue)
```
...
```{r}
only_bankruptcy_differenced <- diff(only_bankruptcy_differenced, lag = 12)
plot(only_bankruptcy_differenced)
acf(only_bankruptcy_differenced, lag.max = 48)
pacf(only_bankruptcy_differenced, lag.max = 100)
```
```{r}
#SARIMA here!!
#Followed by residual diagnostics  
```
